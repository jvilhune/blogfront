...
const App = () => {
  ...
  const handlelikesClick = id => {
    ...
    for(a=0;a<blog.comments.length;a++) {
      tststr[a] = blog.comments[a]
    }
    tststr[a] = 'Great article thank You!'

    //const changedBlog = { ...blog, likes: blog.likes+1 }
    const changedBlog = { ...blog, comments: tststr }
  ...
  const PersonForm = ( {subaction, focuskey, fieldname1, value1, value2, type, text} ) => {
    return (
      <div>
        <form onSubmit={subaction}>
        {fieldname1}<input key="focuskey" autoFocus="autoFocus" type="text "value={value1} onChange={value2} />
        <br />
        <button type={type}>
        {text}
        </button>
    ...
    return (
      <div>
        ...
        {comment}
        <PersonForm subaction={() => addPerson(blog)} focuskey={time} fieldname1='comment: ' value1={newName} value2={handleNameChange} type={'submit'} text='add comment' />
         </div>
